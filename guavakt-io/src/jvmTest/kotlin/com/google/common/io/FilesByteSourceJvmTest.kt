...
class FilesByteSourceJvmTest {
    ...
    fun asByteSource_roundTrip() {
        ...
        val path = dir.resolve("t.txt").toAbsolutePath().toString()
    ...
    fun stringPathSourcesAndSinksUseIncrementalOkioResources() {
        ...
        val output = dir.resolve("incremental.bin").toAbsolutePath().toString()
        Files.asByteSink(output).openSink().use { sink ->
            sink.write(Buffer().writeUtf8("before-flush"), 12L)
            sink.flush()
            assertEquals("before-flush", JFiles.readString(java.nio.file.Path.of(output)))
        }
    }
}
