...
/** Guava ForwardingSortedSetMultimap — forwards to [delegate]. */
abstract class ForwardingSortedSetMultimap<K, V> : SortedSetMultimap<K, V> {
    protected abstract fun delegate(): SortedSetMultimap<K, V>
    ...
    override fun get(key: K): MutableSet<V> = delegate().get(key)
    ...
    override fun asMap(): Map<K, Set<V>> = delegate().asMap()
    ...
    override fun replaceValues(key: K, values: Iterable<V>): Set<V> = delegate().replaceValues(key, values)
    override fun removeAll(key: Any?): Set<V> = delegate().removeAll(key)
    override fun clear() = delegate().clear()
    override fun valueComparator(): Comparator<in V>? = delegate().valueComparator()
