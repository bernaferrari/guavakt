 ...
 *
 * Unlike [Monitor], this class never blocks a thread. State used by a [Guard] must be read and
 * written only inside this monitor's guarded blocks. Every completed block publishes a state
 * change, so waiters re-evaluate their predicates without polling. Cancellation while waiting is
 * non-owning and cannot leak the mutex. No strict FIFO ordering is promised.
 */
class CoroutineMonitor {
    ...
    /**
     * Waits at most [timeout] to acquire a satisfied [guard], then runs [block] without imposing a
     * timeout on the protected work itself. A non-positive timeout still performs one immediate
     * lock-and-guard check.
     */
    ...
    /** Suspends until [guard] is true, then releases the monitor without changing its state. */
