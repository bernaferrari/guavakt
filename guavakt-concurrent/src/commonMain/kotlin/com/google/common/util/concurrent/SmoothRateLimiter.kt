...
/** Shared smooth-rate state used by blocking and coroutine-native rate limiters. */
internal abstract class SmoothRateState(
    nowNanos: Long,
) {
    ...
    /** Whether a reservation can become available within [timeoutNanos], without mutating state. */
    fun canAcquire(nowNanos: Long, timeoutNanos: Long): Boolean =
        nextFreeTicketNanos - timeoutNanos.coerceAtLeast(0L) <= nowNanos

    /** Reserves [permits] and returns how long the caller must wait from [nowNanos]. */
    ...
    protected abstract fun storedPermitsToWaitTime(
...
internal class SmoothWarmingUpState(
    ...
    override fun doSetRate(permitsPerSecond: Double, stableIntervalNanos: Double) {
        ...
        maxPermits = thresholdPermits +
            2.0 * warmupPeriodNanos / (stableIntervalNanos + coldIntervalNanos)
    ...
    private fun permitsToTime(permits: Double): Double =
        stableIntervalNanos + permits * slope
}
