 ...
 *
 * Reservations are serialized per limiter, but waiting suspends without occupying a thread.
 * Bursty limiters save at most one second of unused permits. Warm-up limiters gradually move from
 * a cold interval to the stable rate using the same smooth-ramp model as Guava.
 *
 * Cancellation stops only the caller's wait. Once made, its reservation remains charged. Refunding
 * an arbitrary reservation could move already-issued later reservations ahead of the configured
 * rate, so this explicit policy favors a hard throughput ceiling over cancellation-based bypass.
 * A coroutine cancelled before reservation is not charged.
 *
 * [ticker] must advance on the same time basis as coroutine delays. The default monotonic ticker
 * satisfies that in production; tests can bind it to `TestCoroutineScheduler.currentTime`.
 */
class CoroutineRateLimiter private constructor(
    ...
    /**
     * Atomically reserves when availability is within [timeout], then suspends for that reservation.
     * Negative timeouts are treated as zero, matching Guava's effective behavior.
     */
    ...
    companion object {
        ...
        fun create(permitsPerSecond: Double, ticker: Ticker): CoroutineRateLimiter {
            val now = ticker.read()
            return CoroutineRateLimiter(ticker, SmoothBurstyState(permitsPerSecond, now))
        }
        ...
        fun create(
            ...
            val state = if (warmupPeriod == Duration.ZERO) {
                ...
                SmoothWarmingUpState(
                    permitsPerSecond,
                    now,
                    warmupPeriod.inWholeNanoseconds,
                )
            }
